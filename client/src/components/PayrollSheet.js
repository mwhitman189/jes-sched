  ...
  render() {
    ...
    const sumTeachingMins = () => {
      ...
      if (data.overThresholdOneMins > 0) {
        sums.overThresholdOneAllowance = (Math.round((data.overThresholdOneMins / 60 + Number.EPSILON) * 100)
            / 100)
          * contract.otWageOne;
      }
      if (data.overThresholdTwoMins > 0) {
        sums.overThresholdTwoAllowance = (Math.round((data.overThresholdTwoMins / 60 + Number.EPSILON) * 100)
            / 100)
          * contract.otWageTwo;
      }
      return sums;
    ...
    const grossPayment = contract.baseSalary
